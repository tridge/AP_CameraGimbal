...
// BIN logging is not exercised here
uint64_t ca_binlog_time_us(void) { return 0; }
void ca_binlog_emit(uint8_t id, const void *data, size_t size) { (void)id; (void)data; (void)size; }
void ca_binlog_feedback(const struct ca_gimbal_attitude *attitude) { (void)attitude; }
