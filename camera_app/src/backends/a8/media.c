...
struct ca_media_impl {
    ...
    atomic_bool encoded_frame_logged[CA_A8_VENC_COUNT];
                          ...
                          uint8_t *data, size_t length, uint64_t pts)
{
    ...
    if (channel == CA_A8_MAIN_VENC || channel == CA_A8_SUB_VENC) {
        unsigned stream = channel == CA_A8_MAIN_VENC ? 0U
        ...
        if (channel == CA_A8_MAIN_VENC && !media->isp_bin_loaded &&
            ++media->main_frames >= CA_A8_ISP_BIN_AFTER_FRAMES) {
            pthread_mutex_lock(&media->lock);
            media->isp_bin_loaded = true;
            if (ca_a8_load_isp_bin(CA_A8_ISP_BIN) == 0) {
                ca_log("ISP tuning loaded from %s", CA_A8_ISP_BIN);
            }
            (void)ca_a8_apply_isp_config(&media->config.settings);
...
static void *capture_thread(void *opaque)
{
    ...
    struct pollfd items[CA_A8_VENC_COUNT];

    for (unsigned channel = 0; channel < CA_A8_VENC_COUNT; channel++) {
        items[channel].fd = ca_a8_venc_fd(channel);
        items[channel].events = POLLIN;
    }
    while (!atomic_load(&media->stop)) {
        int ready = poll(items, CA_A8_VENC_COUNT, 100);
        if (ready < 0) {
        ...
        for (unsigned channel = 0; channel < CA_A8_VENC_COUNT; channel++) {
            ...
            if (ca_a8_venc_get(channel, &frame, &length, &pts) == 1) {
...
int ca_media_impl_open(struct ca_media_impl **result, const struct ca_media_config *config)
{
    ...
    ca_video_resolution_size(config->settings.main_resolution,
                             &streams[CA_A8_MAIN_VENC].width,
                             &streams[CA_A8_MAIN_VENC].height);
    streams[CA_A8_MAIN_VENC].codec = config->settings.main_codec;
    ca_video_resolution_size(config->settings.sub_resolution,
                             &streams[CA_A8_SUB_VENC].width,
                             &streams[CA_A8_SUB_VENC].height);
    streams[CA_A8_SUB_VENC].codec = config->settings.sub_codec;
    ca_video_resolution_size(config->settings.recording_resolution,
                             &streams[CA_A8_RECORD_VENC].width,
                             &streams[CA_A8_RECORD_VENC].height);
    streams[CA_A8_RECORD_VENC].codec = CA_VIDEO_H264;
    for (unsigned i = 0; i < CA_A8_VENC_COUNT; i++) {
        streams[i].bit_rate_kbps = stream_bitrate(streams[i].width,
                                                  streams[i].height);
    }
    media->pipeline.jpeg_quality = CA_A8_JPEG_QUALITY;
    stage = "MI pipeline startup";
    if (ca_a8_pipeline_open(&media->pipeline) < 0) goto fail;
    ...
    if (ca_rtsp_open(&media->rtsp, config->rtsp_port, "video1",
                     streams[CA_A8_MAIN_VENC].codec, CA_A8_FRAME_RATE) < 0 ||
        ca_rtsp_add_video(media->rtsp, "video2", streams[CA_A8_SUB_VENC].codec,
                          CA_A8_FRAME_RATE, &media->sub_rtsp_stream) < 0) {
        goto fail;
    }
    ...
    if (ca_live_video_server_open(&media->live_video,
                                  CA_A8_LIVE_VIDEO_PORT) < 0 ||
        ca_live_video_server_configure(
            media->live_video, 0U, streams[CA_A8_MAIN_VENC].width,
            streams[CA_A8_MAIN_VENC].height, CA_A8_FRAME_RATE,
            streams[CA_A8_MAIN_VENC].codec == CA_VIDEO_H264) < 0 ||
        ca_live_video_server_configure(
            media->live_video, 1U, streams[CA_A8_SUB_VENC].width,
            streams[CA_A8_SUB_VENC].height, CA_A8_FRAME_RATE,
            streams[CA_A8_SUB_VENC].codec == CA_VIDEO_H264) < 0) {
        goto fail;
    }
    ...
    (void)ca_a8_venc_request_idr(CA_A8_MAIN_VENC);
    (void)ca_a8_venc_request_idr(CA_A8_SUB_VENC);
    ca_log("media ready: /video1 %ux%u/%u %s; /video2 %ux%u/%u %s; "
           "recording %ux%u H.264; RTSP port %u",
           streams[CA_A8_MAIN_VENC].width, streams[CA_A8_MAIN_VENC].height,
           CA_A8_FRAME_RATE, ca_video_codec_name(streams[CA_A8_MAIN_VENC].codec),
           streams[CA_A8_SUB_VENC].width, streams[CA_A8_SUB_VENC].height,
           CA_A8_FRAME_RATE, ca_video_codec_name(streams[CA_A8_SUB_VENC].codec),
           streams[CA_A8_RECORD_VENC].width, streams[CA_A8_RECORD_VENC].height,
           config->rtsp_port);
    *result = media;
    return 0;
fail:
...
int ca_media_impl_capture_photo(struct ca_media_impl *media, enum ca_photo_scope scope)
{
    ...
    result = ca_a8_capture_jpeg(&jpeg, &length);
    if (result == 0) {
...
{
    if (media == NULL || zoom < 1.0f || zoom > CA_A8_MAX_ZOOM) {
        errno = EINVAL;
        return -1;
    }
    pthread_mutex_lock(&media->lock);
    if (ca_a8_set_zoom(zoom) < 0) {
        pthread_mutex_unlock(&media->lock);
        return -1;
    }
    media->zoom = zoom;
    atomic_store(&media->visible_hfov_deg, ca_video_hfov(CA_VISIBLE_HFOV_DEG, zoom));
    pthread_mutex_unlock(&media->lock);
    return 0;
}
...
int ca_media_impl_autofocus(struct ca_media_impl *media, uint16_t x, uint16_t y)
{
    ...
    /* fixed-focus lens; report success so clients do not retry */
    return 0;
}
...
int ca_media_impl_set_inverted(struct ca_media_impl *media, bool inverted)
{
    ...
    pthread_mutex_lock(&media->lock);
    if (ca_a8_set_inverted(inverted) < 0) {
        pthread_mutex_unlock(&media->lock);
...
{
    pthread_mutex_lock(&media->lock);
    if (!media->isp_bin_loaded) { pthread_mutex_unlock(&media->lock); errno = EAGAIN; return -1; }
    int result = ca_a8_apply_isp_config(settings);
...
int ca_media_impl_exposure(struct ca_media_impl *media, unsigned lens, struct ca_exposure *s)
{
    ...
    typedef int (*query_fn)(uint32_t, uint32_t, void *);
    ...
    int result=query ? query(0,0,&q) : -ENOTSUP;
    if (!result) ca_sstar_exposure_decode(s,&q.info);
    uint32_t value=0;
    int mode_result=mode ? mode(0,0,&value) : -ENOTSUP;
